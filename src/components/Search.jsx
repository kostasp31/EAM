  ...
  return (
    <div>
      ...
      {(userData[0] && userId && userData[0].user_category === "parent") ?
        <div style={{ display: 'flex' }}>
          <div className='box' style={{ width: "300px", height: "1070px", marginTop: "50px", marginLeft: "450px", marginRight: "100px", marginBottom: "50px" }}>
            <h2 style={{ textAlign: 'center', paddingBottom: '10px' }}>Φίλτρα</h2>
            <form>
              <div>
                <h4>Απασχόληση</h4>
                <label>Μερική</label>
                <input style={{ marginLeft: '72px' }} type='checkbox' checked={partApasxolisi} onChange={(e) => setPartApasxolisi(!partApasxolisi)}></input>
                <br />
                <label>Πλήρης</label>
                <input style={{ marginLeft: '70px' }} type='checkbox' checked={fullApasxolisi} onChange={(e) => setFullApasxolisi(!fullApasxolisi)}></input>
                ...
                <input style={{ marginLeft: '99px' }} type='checkbox' checked={parentPlace} onChange={(e) => setParentPlace(!parentPlace)}></input>
                <br />
                <label>Σπίτι Επαγγελματία</label>
                <input style={{ marginLeft: '40px' }} type='checkbox' checked={proPlace} onChange={(e) => setProPlace(!proPlace)}></input>
          ...
          <div style={{ display: 'block', width: "60%" }}>
            {proData && proData.map((data) =>
              <DisplayPro data={data} key={data.id} />
            )}
          </div>
        </div>
        :
